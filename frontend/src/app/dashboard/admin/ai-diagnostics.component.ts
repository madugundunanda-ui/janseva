...
@Component({
  ...
  template: `
    <div class="space-y-6 pb-12">
      <!-- Header Banner -->
      <div class="glass-panel p-6 rounded-2xl border border-var flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden bg-gradient-to-r from-violet-950/10 via-transparent to-transparent shadow-[0_0_30px_rgba(139,92,246,0.1)]">
        <div class="space-y-2">
          <div class="inline-flex items-center gap-2 px-2.5 py-1 rounded-full border border-violet-500/20 bg-violet-950/20 font-mono text-[9px] text-violet-400 uppercase tracking-widest animate-pulse">
        ...
        <div class="flex gap-3">
          ...
          <a [routerLink]="['/dashboard/admin']" class="px-5 py-3 rounded-lg border border-violet-500/30 hover:border-violet-500 text-violet-400 font-mono font-bold text-xs tracking-wider uppercase transition-all duration-300 cursor-pointer">
            ← Back
          </a>
        </div>
      </div>
      ...
      <!-- Status Badge -->
      <div class="glass-panel p-5 rounded-xl border border-var flex items-center justify-between">
        ...
        <span class="font-mono text-[9px] text-muted-var uppercase">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="glass-panel p-5 rounded-xl border border-var">
          <div class="font-mono text-[9px] text-muted-var uppercase tracking-widest mb-2">Total Requests</div>
          <div class="text-2xl font-bold font-mono tracking-tight text-primary-var">{{ healthData?.pythonStats?.totalRequests || 0 }}</div>
          <div class="font-mono text-[8px] text-cyan-400 mt-1 uppercase tracking-wide">LIFETIME AI INVOCATIONS</div>
        </div>

        <!-- Successful -->
        <div class="glass-panel p-5 rounded-xl border border-var">
          <div class="font-mono text-[9px] text-muted-var uppercase tracking-widest mb-2">Successful</div>
        ...
        <!-- Failed -->
        <div class="glass-panel p-5 rounded-xl border border-var">
          <div class="font-mono text-[9px] text-muted-var uppercase tracking-widest mb-2">Failed</div>
          <div class="text-2xl font-bold font-mono tracking-tight" [ngClass]="(healthData?.pythonStats?.failedRequests || 0) > 0 ? 'text-red-400' : 'text-primary-var'">{{ healthData?.pythonStats?.failedRequests || 0 }}</div>
          <div class="font-mono text-[8px] text-red-400 mt-1 uppercase tracking-wide">ERROR RATE</div>
        </div>

        <!-- Avg Latency -->
        <div class="glass-panel p-5 rounded-xl border border-var">
          <div class="font-mono text-[9px] text-muted-var uppercase tracking-widest mb-2">Avg Inference</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="glass-panel p-6 rounded-2xl border border-var space-y-4">
          ...
          <div class="space-y-2.5">
            @for (model of healthData?.models || []; track model) {
              <div class="flex justify-between items-center p-3 rounded bg-white/2 border border-var font-mono text-[10px] uppercase">
                <span class="text-primary-var">🔬 {{ model }}</span>
                <span class="text-emerald-400">LOADED</span>
              </div>
            }
            @if (!healthData?.models || healthData.models.length === 0) {
              <div class="p-3 rounded bg-white/2 border border-var font-mono text-[10px] text-muted-var uppercase text-center">
                No models detected
              </div>
            }
          </div>
          <div class="pt-3 border-t border-white/5 font-mono text-[9px] text-muted-var uppercase space-y-1">
        ...
        <div class="glass-panel p-6 rounded-2xl border border-var space-y-4">
          ...
          <div class="space-y-2.5 font-mono text-[10px] uppercase">
            <div class="flex justify-between items-center p-3 rounded bg-white/2 border border-var">
              <span class="text-primary-var">GPU Accelerator</span>
            ...
            <div class="flex justify-between items-center p-3 rounded bg-white/2 border border-var">
              <span class="text-primary-var">Compute Device</span>
              <span class="text-cyan-400">{{ healthData?.pythonStats?.gpuDeviceName || 'cpu' }}</span>
            </div>
            <div class="flex justify-between items-center p-3 rounded bg-white/2 border border-var">
              <span class="text-primary-var">Node.js Memory (RSS)</span>
              <span class="text-cyan-400">{{ healthData?.system?.nodeMemoryMB || 0 }} MB</span>
            </div>
            <div class="flex justify-between items-center p-3 rounded bg-white/2 border border-var">
              <span class="text-primary-var">Python Memory (WSS)</span>
            ...
            <tbody>
              @for (job of healthData?.system?.activeJobs || []; track job.id) {
                <tr class="border-b border-white/5 hover:bg-white/2">
                  ...
                  <td class="py-2.5 px-3 text-primary-var">{{ job.progress || 0 }}%</td>
                  <td class="py-2.5 px-3 text-muted-var">{{ job.createdAt | date:'short' }}</td>
                </tr>
              } @empty {
                <tr>
                  <td colspan="4" class="py-6 text-center text-muted-var">No recent jobs recorded</td>
                </tr>
              }
            </tbody>
          </table>
