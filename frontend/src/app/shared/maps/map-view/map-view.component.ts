...
@Component({
  ...
  template: `
    <div class="glass-panel p-6 rounded-xl border border-white/10 space-y-6 pb-12 relative overflow-hidden bg-black/40 text-white">
      <!-- Header -->
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 font-mono text-[10px] uppercase">
        <div class="p-3.5 rounded-xl border border-white/10 bg-white/2">
          <span class="text-muted-var block mb-1 text-gray-400">State Integrity Index</span>
          <span class="text-base font-bold text-emerald-400">92.4% SLA</span>
        </div>
        <div class="p-3.5 rounded-xl border border-white/10 bg-white/2">
          <span class="text-muted-var block mb-1 text-gray-400">Audit Transparency</span>
          <span class="text-base font-bold text-cyan-400">100% PUBLIC</span>
        </div>
        <div class="p-3.5 rounded-xl border border-white/10 bg-white/2">
          <span class="text-muted-var block mb-1 text-gray-400">Visual Resolved Audits</span>
          <span class="text-base font-bold text-primary-var">1,482 CLEARED</span>
        </div>
        <div class="p-3.5 rounded-xl border border-white/10 bg-white/2">
          <span class="text-muted-var block mb-1 text-gray-400">Citizen Trust Rating</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div class="lg:col-span-8 relative border border-white/10 bg-white/5 rounded-xl overflow-hidden cursor-crosshair min-h-[500px]">
          <canvas #mapCanvas class="w-full h-full object-cover"></canvas>
          
          <!-- Legend Overlay -->
          <div class="absolute bottom-5 left-5 p-4 rounded-xl bg-black/70 border border-white/10 backdrop-blur-md font-mono text-[8.5px] text-muted-var space-y-2 uppercase pointer-events-none z-10">
          ...
          <!-- Tooltip Overlay -->
          @if (hoveredNode) {
            <div [style.left.px]="tooltipX" [style.top.px]="tooltipY" class="absolute p-4 rounded-xl border border-white/10 bg-black/90 backdrop-blur-md font-mono text-[9px] uppercase tracking-wider text-primary-var shadow-2xl pointer-events-none -translate-x-1/2 -translate-y-full mb-3 space-y-1.5 z-20">
        ...
        <div class="lg:col-span-4 glass-panel p-5 rounded-xl border border-white/10 flex flex-col justify-between bg-black/20">
          <div class="space-y-4">
            ...
            <div data-lenis-prevent class="space-y-3 max-h-[380px] overflow-y-auto pr-1">
              @for (w of wardScores; track w.ward) {
                <div class="p-3 rounded-lg border border-white/10 bg-white/2 space-y-2">
                  ...
                  <div class="flex justify-between font-mono text-[8px] text-muted-var uppercase">
                    <span class="text-gray-400">Active Cases: {{ w.activeCount }}</span>
                    <span class="text-gray-400">Resolved: {{ w.resolvedCount }}</span>
                  </div>
                </div>
              } @empty {
          ...
          <div class="pt-4 border-t border-white/10 font-mono text-[8.5px] text-muted-var uppercase leading-relaxed text-gray-400">
  ...
  private renderMap() {
    const draw = () => {
      ...
      this.wardScores.forEach((w) => {
        ...
        gradient.addColorStop(0, w.color + '33');
