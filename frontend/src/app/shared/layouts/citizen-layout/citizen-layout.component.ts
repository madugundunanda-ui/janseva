        ...
        <div class="space-y-6">
          ...
          <div class="flex items-center gap-3 px-2 py-1">
            ...
            <div>
              ...
              <span class="text-[10px] text-indigo-600 font-semibold uppercase tracking-wider block">Citizen Console</span>
            </div>
          </div>
          ...
          <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-3">
            ...
            <div class="overflow-hidden">
              <div class="font-semibold text-xs text-slate-900 truncate">{{ authService.currentUser()?.name || 'Citizen' }}</div>
              <div class="text-[10px] text-indigo-600 font-medium uppercase">Citizen Portal</div>
            </div>
          </div>

          <!-- Navigation Links -->
          <nav class="space-y-1 text-xs font-medium">
            <a [routerLink]="['/dashboard/citizen']" 
            ...
            <a [routerLink]="['/dashboard/citizen/complaints']" 
               routerLinkActive="bg-indigo-50 text-indigo-700 border-indigo-200 font-semibold" 
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-transparent text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all">
              <span>📋</span>
            ...
            <a [routerLink]="['/dashboard/citizen/settings']" 
               routerLinkActive="bg-indigo-50 text-indigo-700 border-indigo-200 font-semibold" 
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-transparent text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all">
              <span>👤</span>
      ...
      <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-slate-50">
        ...
        <header class="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-10">
          <div class="flex items-center gap-3">
            <h1 class="text-sm font-semibold text-slate-800">Citizen Grievance Portal</h1>
          </div>
